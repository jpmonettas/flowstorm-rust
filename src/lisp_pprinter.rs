...
fn indented_nl (width: usize) -> Vec<PrintToken> {
	...
	for _i in 0..width { v.push(PrintToken::Space) }
	v
}
...
fn n_symbol_print (childs: &Vec<PrintableLispForm>, n: usize, left: usize) -> (Vec<PrintToken>, usize) {
...
fn flatten_print_tokens(tokens: &Vec<PrintToken>) -> Vec<PrintToken> {
	...
	for pt in tokens.iter().cloned() {
		match pt {
			PrintToken::String(_)    => { r.push(pt); }
			PrintToken::BlockOpen{ref val, ref coord}  => { r.push(pt); }
			PrintToken::BlockClose{ref val, ref coord} => { r.push(pt); }
			PrintToken::Atomic{ref val, ref coord}     => { r.push(pt); }
			PrintToken::Space        => { r.push(pt); }  
			PrintToken::Newline      => { r.push(pt); },
...
fn tokens_width (tokens: &Vec<PrintToken>) -> usize {
	tokens
		.as_slice()
		.rsplit(|pt| {if let PrintToken::Newline = pt {true} else {false}})
		.map(|pt_line| {
			pt_line
				.iter()
				.map(|pt| {
					match pt {
						...
						PrintToken::BlockOpen{ref val, ref coord}  => { val.len() }
						PrintToken::BlockClose{ref val, ref coord} => { val.len() }
						PrintToken::Atomic{ref val, ref coord}     => { val.len() }
						PrintToken::Space        => { 1 }  
						PrintToken::Newline      => { 1 },
						PrintToken::PrintTokensVec(v) => panic!("tokens_width only work with a flat Vec<PrintToken"),
					}
				})
				.sum()
...
fn lisp_form_seq_print_tokens (pform: &PrintableLispForm, left: usize) -> Vec<PrintToken> {
    match pform {
		PrintableLispForm::List   {childs, style, coord} |
		PrintableLispForm::Vector {childs, style, coord} |
		PrintableLispForm::Set    {childs, style, coord} => {
			if childs.is_empty() {
				return Vec::new()
			} else {
				match style {
					PrintStyle::Unstyled | PrintStyle::Linear => {
						return join(PrintToken::Space,
									childs
									.iter()
									.map(|lf| PrintToken::PrintTokensVec(lisp_form_print_tokens_aux(lf, left)))
									.collect::<Vec<PrintToken>>())
					},
					
					PrintStyle::Standard => {
						// length of the first thing on the List, Vector, Set
						let (ftoks, flen) = n_symbol_print(childs, 0, left);
						childs
							.iter()
							.enumerate()							
							.map(|(i, pf)| {
								match i {
									0 => PrintToken::PrintTokensVec(lisp_form_print_tokens_aux(pf, left)),
									1 => {
										let mut r : Vec<PrintToken> = Vec::new();
					...
					PrintStyle::Defn => {
						...
						return vec![PrintToken::PrintTokensVec(a_toks),
									PrintToken::Space,
									PrintToken::PrintTokensVec(b_toks),
									PrintToken::Space,
									PrintToken::PrintTokensVec(c_toks),
									PrintToken::Space,
									PrintToken::PrintTokensVec(rest
															   .map(|lf| { // TODO: we can factor this out since it is also used in Standard
																   let mut r : Vec<PrintToken> = Vec::new();
					...
					PrintStyle::PairsBlock => {
						let parts = childs
							...
							.map(|part|{
								...
								match lf1c {
									PrintableLispForm::List{ref childs, ref mut style, ref coord} |
									PrintableLispForm::Vector{ref childs, ref mut style, ref coord} |
									PrintableLispForm::Set{ref childs, ref mut style, ref coord} => {*style = PrintStyle::Linear;},
                                    _ => (),
								};
					...
					PrintStyle::Binding => {
						...
						match bvec_form {                            
							PrintableLispForm::Vector{ref childs, ref mut style, ref coord} => {*style = PrintStyle::PairsBlock;},
                            _ => (),
						};
                        ...
                        vec![PrintToken::PrintTokensVec(symb_toks),
							 PrintToken::Space,
...
fn map_body_print_tokens (pform: &PrintableLispForm, left: usize) -> Vec<PrintToken> {
    if let PrintableLispForm::Map{keys, vals, style, coord} = pform {
		// Kind of hacky but if we interleave keys and vals we can print the map body as a list in PairsBlock style
		return lisp_form_seq_print_tokens (&PrintableLispForm::List{childs: keys
																	.iter()
																	.zip(vals)
																	.flat_map(|(x, y)| {
...
fn lisp_form_print_tokens_aux (pform: &PrintableLispForm, left: usize) -> Vec<PrintToken> {
	match pform {
		PrintableLispForm::List {childs, style, coord} => vec![PrintToken::BlockOpen{val: "(".to_string(), coord: coord.to_vec()},
															   PrintToken::PrintTokensVec(lisp_form_seq_print_tokens(pform, left)),
															   PrintToken::BlockClose{val: ")".to_string(), coord: coord.to_vec()}],
		
		PrintableLispForm::Vector {childs, style, coord} => vec![PrintToken::BlockOpen{val: "[".to_string(), coord: coord.to_vec()},
																 PrintToken::PrintTokensVec(lisp_form_seq_print_tokens(pform, left)),
																 PrintToken::BlockClose{val: "]".to_string(), coord: coord.to_vec()}],
		
		PrintableLispForm::Set {childs, style, coord} => vec![PrintToken::BlockOpen{val: "#{".to_string(), coord: coord.to_vec()},
															  PrintToken::PrintTokensVec(lisp_form_seq_print_tokens(pform, left)),
															  PrintToken::BlockClose{val: "}".to_string(), coord: coord.to_vec()}],
		
        PrintableLispForm::Map {keys, vals, style, coord} => vec![PrintToken::BlockOpen{val: "{".to_string(), coord: coord.to_vec()},
																  PrintToken::PrintTokensVec(map_body_print_tokens(pform, left)),
																  PrintToken::BlockClose{val: "}".to_string(), coord: coord.to_vec()}] ,
		
        PrintableLispForm::String(s) => vec![PrintToken::String(s.to_string())],
		
        PrintableLispForm::Atomic(s, coord) => vec![PrintToken::Atomic{val:s.to_string(), coord: coord.to_vec()}],
	}
...
fn symb_style_lisp_form_deep (pform: &mut PrintableLispForm) {
	match pform {
		PrintableLispForm::List   {childs, ref mut style, coord} => {
			...
			for c in childs {symb_style_lisp_form_deep(c)}
		},
		PrintableLispForm::Vector {childs, style, coord} => for c in childs {symb_style_lisp_form_deep(c)},
		PrintableLispForm::Set    {childs, style, coord} => for c in childs {symb_style_lisp_form_deep(c)},
		PrintableLispForm::Map {keys, vals, style, coord} => {
			for c in keys {symb_style_lisp_form_deep(c)};
			for c in vals {symb_style_lisp_form_deep(c)};
		}, 
...
fn standard_style_next_unstyled_childs(childs: &mut Vec<PrintableLispForm>) -> bool {
	childs.iter_mut().map(|c| standard_style_next_unstyled(c)).fold(false, |acc, n| { acc || n })
}

fn standard_style_next_unstyled (pform: &mut PrintableLispForm) -> bool {	
	match pform {
		PrintableLispForm::List   {ref mut childs, ref mut style, ref coord} => {
		...
		}, // TODO, factor out all this repetitions
		PrintableLispForm::Vector {ref mut childs, style, ref coord} => standard_style_next_unstyled_childs(childs),
		PrintableLispForm::Set    {ref mut childs, style, ref coord} => standard_style_next_unstyled_childs(childs),
		PrintableLispForm::Map {ref mut keys, ref mut vals, style, coord} => {
...
pub fn print_tokens_to_str (tokens: Vec<PrintToken>) -> String {
	...
	for t in tokens {
		match t {
			...
			PrintToken::BlockOpen{val, coord} => r.push_str(&val),
			PrintToken::BlockClose{val, coord} => r.push_str(&val),
			PrintToken::Atomic{val, coord} => r.push_str(&val),
...
fn indented_nl_test() {
	...
	assert_eq!(toks, vec![PrintToken::Newline, PrintToken::Space, PrintToken::Space, PrintToken::Space]);
}

#[test]
fn join_test() {
	assert_eq!(join(PrintToken::Space, vec![PrintToken::String("a".to_string()), PrintToken::String("b".to_string())]),
	           vec![PrintToken::String("a".to_string()), PrintToken::Space, PrintToken::String("b".to_string())])
}

#[test]
fn join_coll_test() {
	assert_eq!(join_coll(vec![PrintToken::Space, PrintToken::Space], vec![PrintToken::String("a".to_string()), PrintToken::String("b".to_string())]),
	           vec![PrintToken::String("a".to_string()), PrintToken::Space, PrintToken::Space, PrintToken::String("b".to_string())])
}

#[test]
fn lisp_form_print_tokens_test() {
	let form1 = read_str("(\"a\", \"b\")").unwrap();
    let form1_toks = lisp_form_print_tokens(&form1);
	assert_eq!(form1_toks,
			   vec![PrintToken::BlockOpen{val: "(".to_string(), coord: vec![]},
					PrintToken::String("a".to_string()),
					PrintToken::Space,
					PrintToken::String("b".to_string()),
			   		PrintToken::BlockClose{val: ")".to_string(), coord: vec![]}]);    
}

#[test]
fn style_lisp_form_test() {
    let mut form1 = read_str("(defn factorial [n] (if (zero? n) 1 (* n (factorial (dec n)))))").unwrap();       
